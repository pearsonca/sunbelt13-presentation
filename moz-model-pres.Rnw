...
#lines(c(0,0),c(0,E_0),col="blue")
#axis(side=2,at=c(0,E_0),col.axis="blue", labels=c(0,expression(E[0])))
@
...
\begin{figure}
\begin{center}
<<plotfig2,fig=TRUE,echo=FALSE>>=
dat <- read.csv('mosquito.csv')
#sagiyama <- dat[which(dat$site == 'Sagiyama'),]
#sagiyama$date <- as.Date(sagiyama$date,"%d-%b-%Y")
#sagiyama$date <- sapply(sagiyama$date,function(date){ as.POSIXlt(as.Date(date,"%d-%b-%Y"))$yday })
natori <- dat[which(dat$site == 'Natori'),]
natori$date <- sapply(natori$date,function(date){ as.POSIXlt(as.Date(date,"%d-%b-%Y"))$yday })

cplot()
lines(x=natori$date-periodT/2,y=natori$moz/max(natori$moz))
@
\end{center}
\end{figure}
