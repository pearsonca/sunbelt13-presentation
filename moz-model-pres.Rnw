...
\begin{figure}
\begin{center}
<<psdeltrig,echo=FALSE,fig=TRUE>>=
PlotStacker(Edeltatrig)
@
\end{center}
\end{figure}
}
...
\begin{figure}
\begin{center}
<<pstrigprop,echo=FALSE,fig=TRUE>>=
PlotStacker(Etrigprop)
@
\end{center}
\end{figure}
}
...
<<psmix,echo=FALSE,fig=TRUE>>=
  refSol <- Msol(Estep)
    ...
    axis(2,seq(0,1.5*MStepPeak,.5*MStepPeak),FALSE)
    axis(3,seq(-periodT/2,periodT/2,10),FALSE)
  for (e in c(Etrap,Etri,Edelta,Edeltatrig,Etrigprop,E2log)) {
    sol <- Msol(e)
    lines(sol[,1],sol[,2],lwd=2,col="lightgray")
  }
  sol <- Msol(Etrig)
  lines(sol[,1],sol[,2],lwd=2)
@
