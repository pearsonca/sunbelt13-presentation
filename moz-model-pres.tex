...
\frame{
What does the last point tell us?  That we need to stub out a connection to an external force.  So, instead of just stating that mosquito populations oscillate, let's propose something else, $E(t)$, does and consider what that means for mosquito population*.  {\em I.e.}, consider solutions to:
...
\frame{
\frametitle{Picking Candidates}
We want to meet those criteria, which mostly amount to ``spikey'' function options, since the sharpness in $E(t)$ will carry over to $M(t)$.  There are a variety of functions to satisfy that.

What else would need to ensure?
...
\frame{
\frametitle{TODO insert poster figs}
}
...
\item a cog for ranges where this kind of model is good (or good enough [for now])
\end{itemize}
}
...
It's a little bit advocacy, and \href{http://github.com/pearsonca/moz-model-pres}{I built this presentation using this work model.}
}
